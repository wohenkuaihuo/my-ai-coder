...
Librarian：从本地 vector_store 检索与需求相关的代码/SQL 片段。
Architect：结合上下文输出结构化修改计划（Markdown）。
Human：人工批准或驳回（LangGraph interrupt_before + UI 恢复执行）。
Coder：按计划生成 JSON 数组，写入目标路径下的 Java 等文件。
QA：在指定目录执行 mvn clean compile，失败则带错重试。
环境要求
...
依赖版本以仓库根目录 requirements.txt 为准（由当前开发环境 pip freeze 导出，可按需精简或升级）。
...
在侧边栏 模型配置 中补全或切换 Chat / Embedding 档案并 启用 一套。
输入 目标项目绝对路径，执行 扫描/初始化（首次或缓存失效时）。
在主界面用 自然语言 描述需求，等待流水线执行；若停在人工审核，请按界面提示 批准或驳回。
5.（可选）命令行初始化
...
配置说明
分层策略
.env：机器级默认值，适合镜像/部署时注入，不进入版本库（见 .gitignore）。
.ai_coder_workspace/config.json：由程序与用户界面读写，可保存 多套 Chat / Embedding 配置及当前启用项。
项目工作区 .ai_coder_workspace/<项目路径哈希>/：每项目独立，包含报告、向量库、聊天记忆等。
向量索引位置
...
Streamlit — Web UI
LlamaIndex — 向量检索与索引
LangGraph — 有状态多步 Agent
OpenAI Python SDK — 兼容多种厂商的 HTTP API
tree-sitter — Java 语法解析
开源与许可
欢迎 Issue / PR。改进方向可包括：更完善的 CLI、非 Maven 项目模板、更细的多模块 Maven 编译路径、以及更安全的密钥托管方案。

本项目默认采用 MIT 许可证，全文见仓库根目录 LICENSE。如需变更版权归属，请修改 LICENSE 中的 Copyright 行。
